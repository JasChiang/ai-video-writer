...
- 可上傳參考檔（圖片、PDF、Markdown）或附加參考網址（`urlContext` 工具直接讀取網頁內容）；上傳的檔案在 Gemini 端保留 48 小時，UI 即時顯示剩餘時間 / 已過期狀態
...
- **草稿自動儲存**：生成成功後即存入 localStorage，最多保留 20 筆（LRU）；儲存內容包含三個標題選項、HTML 全文、SEO 描述、使用的模板 / 顏色 / custom prompt、參考網址與參考影片；若已登入 Google，以帳號的 `sub` 隔離不同使用者的草稿，同一部影片只保留最新一筆
