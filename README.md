...
2. **圖文並茂文章生成**
   ...
   - 一鍵發佈到 Notion，連同截圖時間點與實際圖片一併整理成圖文頁面
...
> 選取資料庫後介面會自動偵測唯一的 Title 欄位名稱，不需手動填入 `Name`。

登入成功後，即使尚未生成文章，介面也會顯示「Notion 匯出」卡片，可先完成授權、選擇資料庫並設定標題。完成文章生成後按下「傳送到 Notion」即可建立新頁面；若勾選「記住 Notion 設定」則會將 Access Token、安全地儲存在瀏覽器的 localStorage 以利下次使用。額外的勾選項目可決定是否將「截圖時間點規劃」與「截圖圖片」同步到 Notion，僅需在資料庫中完成授權即可自動帶入圖文內容。
