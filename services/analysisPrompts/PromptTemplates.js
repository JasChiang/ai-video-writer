...
**開始分析。**`;
  ...
  static buildContentOptimizationPrompt(data) {
    ...
    return `
你是一位專業的 YouTube 頻道顧問，專精於內容優化與流量增長。請根據「頻道數據」與「最近影片清單」，輸出一份格式固定的分析報告，不使用表情符號。
...
## 一、頻道概況摘要
- 以 3-5 點條列，總結頻道規模與整體表現。

## 二、內容健康度診斷
### 2.1 分群結論
- 將 50 支影片分成三類：「表現良好」、「建議優化」、「建議隱藏」。
- 每一類請列出 2-3 句重點判斷依據（不可空泛）。

### 2.2 低效影片建議（3-5 支）
- 請列出 3-5 支最需要處理的影片，並逐支提供：
  - 影片標題
  - 發布日期
  - 目前問題（標題、縮圖、題材、時效性、定位）
  - 建議處置（優化或隱藏）

## 三、優化方向與行動清單
- 依優先順序列出 3-6 項具體可執行措施。
- 每一項需包含：目的、建議動作、預期效益。

## 四、圖表（必填）
請輸出一個 Chart.js JSON code block，格式如下（不可新增其他欄位）：
\`\`\`
{
  "type": "pie",
  "data": {
