...
1. **使用 Markdown 表格**展示所有對比資料（包含健康度評分）
2. **圖表使用規則**：
   - ❌ **禁止使用雷達圖**（不支援）
   - ✅ **可使用 Mermaid 餅圖**展示流量來源分布（語法：\`\`\`mermaid pie ... \`\`\`）
   - ✅ **可使用 Mermaid 四象限圖**展示內容分類（語法：\`\`\`mermaid quadrantChart ... \`\`\`）
   - ✅ **優先使用 Markdown 表格**呈現數據（清晰、易讀）
3. **每個建議必須包含**：
   - 具體執行步驟
   - 預期成效（量化）
   - 執行難度（低/中/高）
...
2. **圖表使用規則**：
   - ✅ **使用 Mermaid 四象限圖**展示分類（語法：\`\`\`mermaid\nquadrantChart\n    x-axis 低觀看 --> 高觀看\n    y-axis 低互動 --> 高互動\n\`\`\`）
   - ✅ **或使用 ASCII 文字圖表**（簡單清晰）
   - ❌ **禁止使用雷達圖等不支援的圖表**
3. **每個建議必須包含**：
   - 具體單元名稱
...
1. **圖表使用規則**：
   - ✅ **可使用 Mermaid 餅圖**展示流量分布（語法：\`\`\`mermaid\npie\n    "推薦流量" : 45\n    "搜尋流量" : 30\n\`\`\`）
   - ✅ **優先使用 Markdown 表格**展示關鍵詞分析
   - ❌ **禁止使用雷達圖、折線圖等不支援的圖表**
2. 使用表格展示關鍵詞分析
3. 每個策略必須包含執行步驟和預期成效
...
1. **使用 Markdown 表格**展示觀眾分布資料（年齡、性別、地理、設備）
2. **圖表使用規則**：
   - ✅ **可使用 Mermaid 餅圖**展示觀眾分布（語法：\`\`\`mermaid\npie\n    "25-34歲" : 40\n    "18-24歲" : 30\n\`\`\`）
   - ❌ **禁止使用雷達圖等不支援的圖表**
   - ✅ **優先使用 Markdown 表格**（更清晰易讀）
3. 提供 3-5 個具體的內容定位建議
