...
-   [ ] **帳號層級速率限制**（`MAX_DOWNLOADS_PER_HOUR = 10`）：
    -   [ ] 執行 1-10 次會觸發影片下載的請求（例如從 URL 生成文章 / 分析影片）。
    -   [ ] 第 11 次請求的任務應以錯誤結束，訊息為「下載次數已達上限…」。
    -   [ ] 應用程式應顯示正確的錯誤與等待提示。
-   [ ] **IP 層級速率限制**（每 IP 每小時 20 次）：
    -   [ ] 同一 IP 總共發出 20 次會觸發下載的請求。
    -   [ ] 第 21 次請求的任務應以「下載次數已達上限…」錯誤結束。
    -   [ ] 應用程式應顯示正確的錯誤與等待提示。
