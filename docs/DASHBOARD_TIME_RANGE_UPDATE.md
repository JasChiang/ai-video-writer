...
> ⚠️ **過時提醒（2026-06-24）**：本文件描述的觀看時間「估算公式（平均時長 × 40%）」是在**尚未整合 Analytics API** 時的做法。現行程式碼已改為直接讀取 YouTube Analytics API 的真實 `estimatedMinutesWatched`，該估算公式僅作為 fallback；時間範圍也已新增「本月／上月」選項。「未來擴展：整合 Analytics API」一節實際上已實作。內文敘述待完整重寫。
