...
> ⚠️ 此端點**不會**自行用 refresh token 換 access token，且受 JWT 保護。你必須先自行取得一個有效的 YouTube `accessToken`，並帶上後端簽發的 session JWT。一般情況建議直接用上面的「方式 A」腳本，它會自動處理 token 交換。

```bash
curl -X POST http://localhost:3001/api/video-cache/generate \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <你的 session JWT>" \
  -d '{
    "accessToken": "ya29_your_youtube_access_token",
...
| Secret 名稱 | 值 | 說明 |
|------------|-----|------|
| `JWT_SECRET` | 與部署環境相同的 JWT 密鑰 | 讓 GitHub Actions 腳本能通過 server 的 JWT 驗證。**必須與你部署的伺服器（Render、Railway、Fly.io 等）上的 `JWT_SECRET` 環境變數值完全相同。** |
| `YOUTUBE_REFRESH_TOKEN` | refresh token 字串 | 取得方式見 [GET_REFRESH_TOKEN.md](./GET_REFRESH_TOKEN.md) |
| `YOUTUBE_CLIENT_ID` | OAuth client ID | Google Cloud Console 取得 |
| `YOUTUBE_CLIENT_SECRET` | OAuth client secret | Google Cloud Console 取得 |
| `YOUTUBE_CHANNEL_ID` | `UCxxxxxxxxxxxxxxxxxx` | 你的 YouTube 頻道 ID |
| `VIDEO_CACHE_GIST_TOKEN` | `ghp_xxxxxxxxxxxxxx` | 有 gist 權限的 GitHub Personal Access Token |
| `VIDEO_CACHE_GIST_ID` | `abc123...` | 首次執行後取得的 Gist ID |
| `VIDEO_CACHE_GIST_FILENAME` | `youtube-videos-cache.json` | Gist 內的檔案名稱 |
...
### API 回傳 500 SERVER_MISCONFIGURED

**症狀**：日誌顯示 `API 請求失敗: 500 {"error":"SERVER_MISCONFIGURED"}`

**原因**：GitHub Secrets 缺少 `JWT_SECRET`，server 無法初始化 JWT 驗證。

**解決方式**：
1. 前往 repository「Settings」→「Secrets and variables」→「Actions」
2. 新增 `JWT_SECRET`，值必須與你部署的伺服器上的 `JWT_SECRET` 環境變數相同
...
- [ ] 取得 Gist ID
- [ ] 在 GitHub Secrets 設定所有必要的變數（包含 `JWT_SECRET`，值必須與部署伺服器一致）
- [ ] 推送程式碼到 GitHub
- [ ] 手動執行 workflow 測試成功
- [ ] 確認 cron 時間設定正確
- [ ] 設定 token 更新提醒
